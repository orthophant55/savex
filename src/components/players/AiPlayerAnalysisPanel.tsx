...
import { streamPlayerAnalysis, type StreamStatus } from "@/lib/api/ai";
import ArticleBody from "@/components/articles/ArticleBody";

interface AiPlayerAnalysisPanelProps {
  playerId: string;
  playerName: string;
}

export default function AiPlayerAnalysisPanel({
  playerId,
  playerName,
}: AiPlayerAnalysisPanelProps) {
  ...
  const start = useCallback(async () => {
    ...
    try {
      for await (const chunk of streamPlayerAnalysis(playerId, abortRef.current.signal)) {
      ...
      {status === "idle" && (
        <>
          <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-4">
            AI가 세이버매트릭스 지표를 종합해 선수 분석 리포트를 생성합니다.
            강점·약점·최근 흐름·회귀 가능성을 포함한 심층 분석입니다.
          </p>
          <button
            onClick={start}
